...
import '../orders.css'
import Procument from "../Modal/procument"
let procument = new Procument();
const db = firebase.ref();
export default class ApproveOrderService extends Component{
    constructor(props) {
        super(props);
        this.state={
            Order:[],
    ...
    componentDidMount() {
        ...
        procument.getOrderID(this.props.match.params.id);
        this.handleOrderObject();
        this.handleItemsObject();
                    ...
                    if(rr.item == ss.item){
                        ...
                        if(ss.unit == undefined){
                            ...
                            this.setState({
                                ...
                                unitError:'Unit value of a order item not set'
                            })
                        }else{
    ...
    handleOrderObject() {
        this.setState({
            orderList: procument.getOrder()
        })
    }

    handleItemsObject(){
        this.setState({
            Item:procument.getItems()
        })
    }
    ...
    sendDetails(){
        let templateParams = {
            from_name: this.state.supplierEmail  ,
            to_name: "elorakate@gmail.com",
    ...
    approveOrder(){
        ...
        if(this.state.totPrice < this.state.budgetLimit && this.state.unitPriceInvalid == false){
            ...
            this.setState({
                showMessage:"Order Approved and email has been sent"
            })
        }else{
            console.log("budget limit not exceed")
            this.setState({
                errorq:"Total Cost Exceed Budget Limit ! Cannot Approve"
            })
                    ...
                    <div style={{height:'40%',width:'70%',marginLeft:'15%',marginRight:'15%'}} className="mt-3 mb-3">
                        <Table responsive="md" className="react-bs-table">
                                    ...
                                    return   <tr><td style={{paddingRight:'5%'}} />
                                        <div className="row">
                                            <div className="col-6" align="center">
                                                <Button variant="primary" onClick={()=>{this.approveOrder()}}>Approve</Button>
                                            </div>
                                            <div className="col-6" align="center">
                                                <Button variant="danger"  onClick={()=>{this.declinedOrder()}}>Decline</Button>
                                            </div>
                                        </div>
                                    </tr>
                                }
                            ...
                            <tr>
                                ...
                                <div className="row" align="center" style={{ fontSize: 16, color: "green" }}>
                                    {this.state.showMessage}
                                </div>
                                <div className="row" align="center" style={{ fontSize: 16, color: "red" }}>
